...
def normalize_required_items(raw_count):
    if raw_count in (None, ""):
        return 0
    try:
        decimal_value = Decimal(str(raw_count)).quantize(Decimal("1"), rounding=ROUND_HALF_UP)
...
def print_questionnaire(request, group_id):
    ...
    for entry in tos_entries:
        for category in category_list:
            total_required_items += normalize_required_items(getattr(entry, category, 0))

    TARGET_ITEM_COUNT = total_required_items
    request.session['target_item_count'] = TARGET_ITEM_COUNT
    ...
    if len(question_answer_pairs) < TARGET_ITEM_COUNT:
        messages.error(
            request,
            f"Unable to generate {TARGET_ITEM_COUNT} questions. Please ensure each category has enough available items.",
        )
    total_pairs = len(question_answer_pairs)

    if total_pairs >= TARGET_ITEM_COUNT and TARGET_ITEM_COUNT > 0:
...
#         # Process each category for this entry
#         for category in category_list:
#             required_items = normalize_required_items(getattr(entry, category, 0))
#             if required_items <= 0:
#                 continue

#             # Get available questions for this specific combination
#             available_questions = list(
#                 Questionnaire.objects.filter(
#                     status=0,
#                     subject=subject,
...
#                     subtopic=subtopic,
#                     category__category=category,
#                 ).exclude(id__in=used_question_ids)  # Exclude already used questions
#             )

#             if not available_questions:
#                 logger.warning(
#                     f"Not enough questions for {subject.subject_name} - {topic.topic_name} - "
#                     f"{subtopic.subtopic_name} - {category}. Required: {required_items}, Available: 0"
#                 )
#                 continue

#             # Shuffle and select the required number of questions
#             rd.shuffle(available_questions)
#             selected_questions = available_questions[:required_items]
...
#     total_generated = len(question_answer_pairs)
#     if total_generated < TARGET_ITEM_COUNT:
#         messages.warning(
#             request,
#             f"Generated {total_generated} questions out of {TARGET_ITEM_COUNT} required. "
#             f"Some categories may not have enough available questions."
#         )

#     # Shuffle all generated questions
#     rd.shuffle(question_answer_pairs)

#     # Assign numbers and prepare final data
#     for index, (question_entry, answer_key_entry) in enumerate(question_answer_pairs, start=1):
#         question_entry['number'] = index
#         generated_questions.append(question_entry)
#         answer_key_entry.number = index
#         answer_keys_tos.append(answer_key_entry)
...
def save_answer_key_toss(request):
    ...
    target_item_count = request.session.get('target_item_count', 100)

    if questions == target_item_count:
        ...
        messages.error(
            request,
            f"Save failed. Only {target_item_count} questions are required to proceed.",
        )
...
def print_questionnaire_view_table(request, group_id):
    ...
    for entry in tos_entries:
        for category in ['understanding', 'remembering', 'analyzing', 'creating', 'evaluating', 'applying']:
            count = normalize_required_items(getattr(entry, category))
...
def masterfilestudentscreate(request):
    ...
    if request.method == 'POST':
        ...
        posted_studentid = (request.POST.get('studentId') or '').strip()
